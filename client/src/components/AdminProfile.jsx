...
const AdminProfile = () => {
  ...
  const [cookies] = useCookies(['adminemail']);
  ...
  useEffect(() => {
    const fetchAdminData = async () => {
      try {
        ...
        const filtered = data.filter((admin) => admin.email === adminEmail);
                ...
                <div key={admin._id} className="col-12 col-md-6">
                  <div className="card product-card mb-3">
                    <div className="card-body">
                      <p className="product-title d-block">Email: <b>{admin.email}</b></p>
                    </div>
                  </div>
                  <button className="btn btn-danger" onClick={() => LoadEdit(admin._id)}>Update</button>
                </div>
              ))}
            </div>
          </div>
