...
const AgentLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/api/v1/agent/login', {
        email: email,
        password: password,
      });
      ...
      if (response.status === 200) {
        ...
        setCookie('agentemail', response.data.agent, { path: '/', sameSite: 'strict' });
setCookie('location', response.data.agentLocation, { path: '/', sameSite: 'strict' });
setCookie('mobile1', response.data.mobile1, { path: '/', sameSite: 'strict' });
        setError('');
  ...
  return (
    <div>
      <title>Web Based Vehicle Insurance Management System</title>
